...
На этой схеме изображена локальная сеть 192.168.0.0/24 с двумя подсетями 192.168.0.8/29 и 192.168.0.16/29 - все хорошо:

![](./img/9_1.png)

### Бред

На самом деле, на виртуальной машине сложно разбираться с сетями. ФИЗИЧЕСКИЙ интерфейс в ВИРТУАЛЬНОЙ машине - вводит в ступор.
Если отойти от виртуальных машин, то физический интерфейс - это, например, сетевая карта. Она однозначно идентифицируется
MAC-адресом и при этом может иметь несколько IP-адресов.

В остальном на картинке изображен бред. Если создать на физическом интерфейсе два VLAN, то адреса в них пересекаться не могут.
При этом адреса не могут пересекаться с адресами вне VLAN (192.168.0.1/24 фигурирует в трех местах на изображении).

![](./img/9_2.png)

### VLAN

Факт 1. VLAN настраивается только на комутаторах/маршрутизаторах. При этом хосты ничего не знают про VLAN.
Пунктиром на изображении показан VLAN. 
Факт 2. В VLAN входят хосты:
- 192.168.0.10/29
- 192.168.0.11/29
- 192.168.0.18/29

Факт 3. 192.168.0.18/29 находится в другой локальной сети. И это никак не мешает хостам общаться.

Факт 4. Хост 192.168.0.18/29 МОЖЕТ отправить данные на хост 192.168.0.14/29.

Факт 5. Хост 192.168.0.14/29 НЕ МОЖЕТ отправить данные на хост 192.168.0.18/29.
